...
function getCounty(){
var txt = '<option value = "0">' + AddressArray[0][0];
for ( i = 1 ; i < AddressArray.length ; i++){
txt += '<option value = "' + i + '">' + AddressArray[i][0];
}
document.getElementById("county").innerHTML = txt;
getTown();
}

function getTown(){
txt = '<option value = "0">選鄉鎮</option>';
var county = document.getElementById("county").value;
for (var j = 1 ; j < AddressArray[county].length ; j++){
txt += '<option value = "'+ AddressArray[county][j].substring(0,3) +'">' + AddressArray[county][j].substring(3);
}
...
document.getElementById("fullAddress").value = county.options[county.selectedIndex].text + town.options[town.selectedIndex].text;
